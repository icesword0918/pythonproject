def fun_checkout(money):
    """
    功能：计算商品合计金额并进行折扣处理
    :param money:保存商品金额列表
    :return:返回商品的合计金额和折扣后金额
    """
    ...
    return money_old,money_new      # 返回总金额和折扣金额
...
while True:
    # 请不要输入非法的金额，否则将抛出异常
    inmoney=float(input("请输入商品金额（输入0标识输入完毕）："))
    if int(inmoney)==0:
        break     # 退出循环
    else:
        list_money.append(inmoney)    # 将金额添加到金额列表中
money=fun_checkout(list_money)        # 调用函数
print("合计金额：",money[0],"折扣金额：",money[1])   # 显示应付金额
